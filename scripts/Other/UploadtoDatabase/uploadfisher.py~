...
try:
    ...
    datafile1 = open('./Stuff/fish.csv', 'r')  #opens file listing the datafile
    productreader = csv.reader(datafile1) #load datafile in a productreader
    for row in productreader:
	   Item=[]
	   Item=["".join(row[2].split()),row[3],row[5]]+GetCategory(row[10])+[row[9]]
 	   if (len(row[9])<5):   #if the url is less than 5 characters
		Item[7]='./style/img/Icons/'+"".join(Item[3].split())+'.jpeg' 	   			   
	   
	   cur.execute("INSERT INTO Fisher VALUES(%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE Name=%s,Price=%s,Cat1=%s,Cat2=%s,Cat3=%s,Cat4=%s,URL=%s",Item+Item[1:] ) #(Name,Description,Price,Manufacturer,URL,Subgroup1,Subgroup2,Subgroup3,Subgroup4,ID)  
	   con.commit()
